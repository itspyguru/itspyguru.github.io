...
export default function Window({ win }: { win: WinSpec }) {
  ...
  return (
    <div className="os-window" style={{ left: pos.x, top: pos.y, zIndex: win.z, width: win.type === 'text' ? 'min(560px,92vw)' : 430 }} onMouseDown={() => focus(win.id)}>
      <div className="titlebar bg-surface-container-highest px-3 py-2 border-b border-outline-variant/50 flex justify-between items-center" onPointerDown={down} onPointerMove={move} onPointerUp={up}>
        <span className="text-data-label font-data-label text-primary-fixed-dim flex items-center gap-2 truncate"><span className="material-symbols-outlined text-sm">{node.icon || 'folder'}</span>{win.type === 'text' ? node.name : '~/' + segs.join('/')}</span>
